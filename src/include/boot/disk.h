...
#define INIT_REQUEST_QUEST_HEAD(head)  \
    do {                               \
        (head)->count = 0;             \
        (head)->use = NULL;            \
...
struct disk_identify_info_struct {
    ...
    //	47	15:8 	80h
    //		7:0  	00h=Reserved
    //			01h-FFh = Maximumnumber of logical sectors that shall be transferred per DRQ data block on READ/WRITE
    // MULTIPLE commands
    ...
    //		7:0	xxh current setting for number of logical sectors that shall be transferred per DRQ data block on
    ...
    //	93	Hardware resset result.The contents of bits (12:0) of this word shall change only during the execution of a
    // hardware reset.
