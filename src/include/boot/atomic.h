...
{
    asm volatile("lock subl %1, %0" : "+m"(atomic->counter) : "ir"(i));
}

static inline void atomic_inc(atomic_t *atomic)
{
    asm volatile("lock incl %0" : "+m"(atomic->counter));
}

static inline void atomic_dec(atomic_t *atomic)
{
    asm volatile("lock decl %0" : "+m"(atomic->counter));
}
...
{
    asm volatile("lock andl %1, %0" : "+m"(atomic->counter) : "ir"(~(mask)));
}

static inline void atomic_set_mask(int mask, atomic_t *atomic)
{
    asm volatile("lock orl %1, %0" : "+m"(atomic->counter) : "ir"(mask));
}
