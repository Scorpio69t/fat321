...
static void cpu_gdt_init(void)
{
     ...
     */
    init_descriptor(get_desc(gdt, SELECTOR_FLAT_C), 0x00, 0xfffff, DA_LIMIT_4K | DA_32 | DA_CR);

    /*
     * ring0数据段描述符，寻址空间0x00000000 - 0xffffffff(4GB), 可读可写
     */
    init_descriptor(get_desc(gdt, SELECTOR_FLAT_RW), 0x00, 0xfffff, DA_LIMIT_4K | DA_32 | DA_DRW);

    /*
     * ring3显示段描述符，寻址空间0x000b8000 - 0x000c7fff, 可读可写
     * 用于在屏幕上显示
     */
    init_descriptor(get_desc(gdt, SELECTOR_VIDEO), 0xb8000, 0xffff, DA_DRW | DA_DPL3);
